...
def pregunta_01():
    ...
    # Retorne `X` y `y`
    print(X.head())  # Muestra las primeras filas de X (sin la columna 'charges')
    print(y.head())  # Muestra las primeras filas de y (solo la columna 'charges')
    return X, y
...
def pregunta_02():
    ...
    (X_train, X_test, y_train, y_test) = train_test_split(
        X,
        y,
        test_size=300,  # Cambia el valor según tus necesidades, aquí se usa el 20% como conjunto de prueba
        random_state=12345,
    )
...
def pregunta_03():
    ...
    # Busque la mejor combinación de regresores
